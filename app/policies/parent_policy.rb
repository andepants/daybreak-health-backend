...
# Authorization policy for Parent model
#
...
#
# PHI Access Rules:
# - Parent data contains PHI (name, email, phone)
# - Only session owner can create/update their parent data
# - Coordinators can view for care coordination
# - Admins and system have full access
#
# Example usage in GraphQL:
#   class UpdateParent < BaseMutation
...
class ParentPolicy < ApplicationPolicy
  # Session owner, admin, or system can create parent data
  ...
  # Session owner, coordinator, admin, or system can view parent data
  # AC 2.6.4: PHI access control
  #
  # @return [Boolean] true if user has permission
  def show?
  ...
  # Session owner, admin, or system can update parent data
  # AC 2.6.4: PHI access control
  #
  # @return [Boolean] true if user has permission
  def update?
  ...
  # Parent data should not be destroyed, only soft deleted via session
  #
  ...
  # Coordinators, admins, and system can list parents
  #
  # @return [Boolean] true if user has permission
  def index?
  ...
  class Scope < ApplicationPolicy::Scope
    # Return parents based on user role
    # AC 2.6.4: Role-based PHI scoping
    #
    # @return [ActiveRecord::Relation] Scoped parents
    def resolve
      return scope.none if user.blank?

      case user[:role]&.to_s
      when 'system', 'admin', 'coordinator'
        # System, admin, and coordinators can see all parents
        scope.all
      when 'parent', 'anonymous'
        # Parents and anonymous users can only see their own parent data
  ...
  # Check if current user owns the session associated with this parent
  #
